...
export default function PortfolioUniverse() {
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      <motion.div className="fixed inset-0 -z-10" style={{ filter: blur.to((b) => `blur(${b}px)`) }}>
        <motion.div
          className="absolute inset-0"
          style={{ background: "radial-gradient(circle at var(--lx) var(--ly), rgba(130,145,255,0.25), transparent 45%)" } as React.CSSProperties}
          animate={{ "--lx": `${dynamicLightX.get()}%`, "--ly": `${dynamicLightY.get()}%` } as never}
